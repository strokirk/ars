...
export function renderSheet(ch: Character, b: Budgets = computeBudgets(ch)): string {
  ...
  L.push(`**Concept:** ${ch.concept || "—"} · **Age:** ${ch.age} (fresh Gauntlet unless noted)`);
  ...
  L.push(`(xp: childhood ${b.childhood.spent}/${b.childhood.cap} · later life ${b.laterLife.spent}/${b.laterLife.cap} · apprenticeship ${b.apprenticeship.spent}/${b.apprenticeship.cap})`, "");
  ...
  L.push("## Personality / Reputation / Confidence");
  const traits = ch.personality.map((p) => `${p.trait} ${fmt(p.value)}`).join(", ") || "—";
  const conf = confidenceScore(ch);
  L.push(`Traits: ${traits} · Reputation: ${ch.reputation ?? "—"} · Confidence ${conf.score} (${conf.points} points)`, "");
