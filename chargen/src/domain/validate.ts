...
// warnings, not errors: we don't know what house rules a given troupe runs, or which
// Virtue might legitimately unlock something the data doesn't model. They're flagged
// (⚠, and in this set) but never block a mutation or final legality. Contrast
// "incomplete" issues (under-spend, imbalance, a required pick not yet made, a
// mandatory minimum unmet) which stay errors — those are unfinished, not overruled.
...
export function validate(ch: Character, mods: Modifiers = deriveModifiers(ch), budgets: Budgets = computeBudgets(ch, mods)): Issue[] {
  ...
  if (c.over) warn("char-over", "characteristics", `Characteristics overspent: ${c.spent} of ${c.cap} points used.`);
  ...
  if (vf.flawPoints > vf.cap) warn("flaw-cap", "virtues-flaws", `Flaw points ${vf.flawPoints} exceed the ${vf.cap}-point limit${kind === "grog" ? " for grogs" : ""}.`);
  if (!vf.balanced) err("vf-balance", "virtues-flaws", `Virtue points (${vf.virtuePoints}) must equal Flaw points (${vf.flawPoints}).`);
  if (vf.minorFlaws > 5) warn("minor-flaw-cap", "virtues-flaws", `Too many Minor Flaws: ${vf.minorFlaws} (max 5).`);

  if (kind === "grog") {
    // Grogs are minor characters: Minor V/F only, no Story Flaws, no The Gift, ≤1 Personality Flaw.
    if (vf.majorVirtues + vf.majorFlaws > 0) warn("major-forbidden", "virtues-flaws", `Grogs may not take Major Virtues or Flaws.`);
    if (vf.storyFlaws > 0) warn("story-forbidden", "virtues-flaws", `Grogs may not take Story Flaws.`);
    if (vf.personalityFlaws > 1) warn("pers-flaw-cap", "virtues-flaws", `Grogs take at most 1 Personality Flaw (have ${vf.personalityFlaws}).`);
    if (ch.virtues.some((v) => /^the gift$/i.test(v.name))) warn("gift-forbidden", "virtues-flaws", `Grogs may not have The Gift — a Gifted character is too important to be a grog.`);
  } else {
    if (vf.storyFlaws > 1) warn("story-flaw-cap", "virtues-flaws", `Too many Story Flaws: ${vf.storyFlaws} (max 1).`);
    if (vf.personalityFlaws > 2) warn("pers-flaw-cap", "virtues-flaws", `Too many Personality Flaws: ${vf.personalityFlaws} (max 2).`);
    if (vf.majorPersonalityFlaws > 1) warn("major-pers-flaw", "virtues-flaws", `Too many Major Personality Flaws: ${vf.majorPersonalityFlaws} (max 1).`);
  }
  ...
  if (isMagus) {
    if (vf.majorHermeticVirtues > 1) warn("major-herm-virtue", "virtues-flaws", `Too many Major Hermetic Virtues: ${vf.majorHermeticVirtues} (max 1).`);
  ...
  // 4. Later life.
  if (budgets.laterLife.over) warn("later-over", "later-life", `Later-life xp overspent: ${budgets.laterLife.spent}/${budgets.laterLife.cap}.`);
  ...
  if (isMagus) {
    ...
    if (ap.over) warn("appr-over", "apprenticeship", `Apprenticeship xp overspent: ${ap.spent}/${ap.cap}.`);
